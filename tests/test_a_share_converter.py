...
def test_batch_conversion_continues_after_an_invalid_source(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    assert "source_data" in report.read_text(encoding="utf-8")
...
def test_non_positive_adjusted_price_is_reported_as_a_source_problem(tmp_path: Path) -> None:
    source = tmp_path / "SH#600036.txt"
    source.write_bytes(
        "600036 测试股票 日线 前复权\r\n"
        "日期 开盘 最高 最低 收盘 成交量 成交额\r\n"
        "06/03/2015 -1.66 -1.49 -1.67 -1.53 100 1000\r\n"
        "#数据来源:通达信\r\n".encode("gb18030")
    )

    with pytest.raises(SourceDataError, match="非正价格") as exc_info:
        convert_a_share_file(source, tmp_path / "SH600036.csv")

    assert exc_info.value.code == "non_positive_price"
